...
# Routing:
# 	Another way to configure router is through the add_url_rule() method
...
def candidate_by_id(id):
	candidate = DATA_PROVIDER.get_candidates(id)
	if candidate:
...
# ROUTING
# 	This method updates the name of a candidate, which was found by its ID.
...
# 	which converts the request parameter to an integer value.
#
...
def random(nrOfItems):
	...
	return jsonify({"candidate": candidates, "total":len(candidates)})

# ROUTING
# 	This is a method which deletes the candidates with the given id.
...
# ROUTING
# 	Here we use the HTTP POST verb and the Flask request object to get
# 	the data which was sent from the client.
#
# 	Please note, that the response contains the ID of the newly added 
# 	candidate and the URL through which this can be accessed.
